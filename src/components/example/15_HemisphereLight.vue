...
<script>
//页面:特殊光源THREE.HemisphereLight户外光照光源
...
export default {
  ...
  methods: {
    ...
    initLight() {
      ...
      this.hemiLight = new THREE.HemisphereLight(0x0000ff,0x00ff00,0.3);
    ...
    initGui() {      
        ...
        datGui.addColor(this.gui,"skyColor").onChange(function (e) {
            window.vue.hemiLight.color = new THREE.Color(e);
        });
        datGui.addColor(this.gui,"groundColor").onChange(function (e) {
            window.vue.hemiLight.groundColor = new THREE.Color(e);
        });
        datGui.add(this.gui,"hemiLightIntensity",0,1).onChange(function (e) {
            //min:0  max:1
            window.vue.hemiLight.intensity  = e;
        });        

        datGui.addColor(this.gui,"directionalLight").onChange(function (e) {
            window.vue.directionalLight.color =  new THREE.Color(e);
        });
        datGui.add(this.gui,"directionalLightIntensity",0,5).onChange(function (e) {
          //min:0  max:5
          window.vue.directionalLight.intensity  =  e;
        });
        ...
        datGui.add(this.gui,"debug").onChange(function (e) {
            if(e){
               ...
               var debugs = window.vue.scene.getObjectByName("debug");
               window.vue.scene.remove(debugs);
            }
        });     
    },
    initModel() {
